...
答错的题**当天不再出现**，隔天再见，之后逐级拉长：

```
答错 ──→ 1 天 ──→ 3 天 ──→ 7 天 ──→ 16 天 ──→ 37 天 ──→ 85 天 …
         ↑                                     （之后按 EF 自动拉长）
         └───── 中间再答错就退回这里重新爬
```

适合考研这种要**长期记住**的场景——记忆要在快忘掉的边缘复习才记得牢，
当天反复刷主要得到的是短期记忆。

**② 当天巩固**

Anki 的传统做法：答错的题本轮内再练一遍，分钟级重复。

```
答错 ──→ 本轮内重考（最多 3 次）──→ 1 分钟 ──→ 10 分钟 ──→ 次日 ──→ …
```

适合时间紧、想当天就把一道题吃透。

> 切换模式时会自动处理已有数据：从「当天巩固」切回「遗忘曲线」时，
> 那些还排在 1～10 分钟后的题会被顺延到次日，不会立刻冒出来。
...
实测数据（基于真实提示词长度 + 题库里的真实题目）：

| 场景 | 输入 token | 输出 token | 合计 |
|---|---|---|---|
| 平均（参考 3 道错题） | ~990 | ~225 | **~1,200** |
| 最坏（挑解析最长的 3 道） | ~1,440 | ~430 | **~1,870** |
| 只参考 1 道错题 | ~630 | ~225 | **~850** |
| 开了深度思考 | 同上 | 800~1,500+ | ~1,700~2,500 |

按 `deepseek-flash` 的官价（非高峰时段，1M token：输入 $0.15 / 输出 $0.6）：

```
平均情况      约 $0.00028 / 道     100 道约 $0.028（≈ 0.2 元）
每天 5 道     约 $0.0014 / 天      一个月约 $0.04（≈ 0.3 元）
```
...
- `deepseek-v4-pro` 大约是 `deepseek-flash` 的 **4 倍**价格。
- **高峰时段价格翻倍**：UTC 01:00-04:00 和 06:00-10:00（北京时间 **09:00-12:00 和 14:00-18:00**，
  周一至周五）。恰好是白天学习的时段。不过就算翻倍也还是几分钱级别。
- **开启深度思考会明显变贵**（输出侧多算思考过程，通常多 2~5 倍），
  所以默认关掉它。如果服务端不接受这个参数而退回默认设置，你会看到出题变慢、费用变高。
- 系统提示词每次都完全相同，会命中 DeepSeek 的上下文缓存，那部分几乎不花钱。

想自己重算（改了提示词之后）：

```powershell
C:/Python314/python.exe tools/estimate_ai_cost.py
```
